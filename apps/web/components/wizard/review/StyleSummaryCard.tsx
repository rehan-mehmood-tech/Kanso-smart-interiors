...
interface StyleSummaryCardProps {
  styleName: string;
  /** Budget tier label, when one has been chosen. */
  budgetName?: string | null;
}

export function StyleSummaryCard({ styleName, budgetName }: StyleSummaryCardProps) {
  return (
    <div className="bg-surface-container-lowest rounded-xl p-6 shadow-[0px_8px_24px_rgba(0,0,0,0.04)] border border-outline-variant flex flex-col md:flex-row justify-between md:items-center gap-4">
      <div>
        ...
        {budgetName && (
          <p className="text-body-md font-body-md text-secondary mt-1">{budgetName}</p>
        )}
      </div>
